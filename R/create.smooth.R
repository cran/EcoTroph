...
tab_smooth <- array(dim=c(length(TL_out),length(ecopath$group_name)),dimnames=list(TL_out,Troph_round))
...
if (smooth_type==2)
{
sigmaLN <- smooth_param*log(Troph_round-0.05)        ##lognormal sigma (other choices : constant, OI or saturation function) 
...
#Handle of the zero-value in sigmaLN (notably for sigmaLN=OI)
for (i in 1:length(ecopath$group_name))
{
if(sigmaLN[i]==0) 
{
sigmaLN[i]<- 0.01
cat(paste("the value of the sigmaLN was 0 for the group", ecopath$group_name[i],". We change it for a value of 0.01. If you want to correct it, change your dataset (fix() or else).\n"))
} 	
}
