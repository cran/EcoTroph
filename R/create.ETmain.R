...
{
#ecopath=ecopath_guinee
#smooth_type = NULL; sigmaLN_cst = NULL; pas = NULL; shift = NULL; smooth_param = NULL
...
N_loss <- c(log(P[-length(P)]/P[-1])/((V <- as.numeric(rownames(biomass)))[-1] - V[-length(V)]) - F_loss[-length(F_loss)], NA)
Fish_mort <- Y_tot/B
Fish_mort_acc <- Fish_mort/(B_acc/B)
Selec <- B_acc/B
Time <- cumsum(c(0, ((V <- as.numeric(rownames(biomass)))[-1] - V[-length(V)])/Kin[-length(Kin)]))
N_loss_acc <- c(log(P_acc[-length(P_acc)]/P_acc[-1])/((V <- as.numeric(rownames(biomass)))[-1] - V[-length(V)]) - F_loss_acc[-length(F_loss_acc)], NA)
